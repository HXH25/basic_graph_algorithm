...
// 关联矩阵转邻接矩阵
std::vector<std::vector<int>> incidence_matrix::in_to_ad() const
{
  // 关联矩阵的行数是顶点数，列数是边数
...
//检测是否为无向图
bool adjacency_matrix::if_nonarrow()
{
...
tree::tree(incidence_matrix graph_matrix, incidence_matrix tree_matrix)
{
  ...
  const int cut_set_rows = num_edges - extra_edge_num;
  std::vector<std::vector<int>> cut_set_matrix(cut_set_rows, std::vector<int>(num_edges, 0));

  // 填充前 extra_edge_num 列
  for (int i = 0; i < cut_set_rows; i++)
  {
  ...
  // 应用逆置换，恢复原始列顺序
  std::vector<std::vector<int>> final_cut_set(cut_set_rows, std::vector<int>(num_edges, 0));
  for (int i = 0; i < cut_set_rows; i++)
  {
